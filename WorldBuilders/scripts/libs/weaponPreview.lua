...
-- TODO:
-- second ones cause first to stop?
-- Seems like second ones aren't triggering right...
...
local STATE_FINAL_EFFECT = 4
local STATE_SECOND_TARGET_AREA = 5
...
local function clearMarks(state)
	if state then
		...
		clearMarks(STATE_SECOND_TARGET_AREA)
		clearMarks(STATE_SKILL_EFFECT)
		clearMarks(STATE_FINAL_EFFECT)
...
local function onMissionUpdate()
	...
	-- GOING THROUGH HERE
	-- THis also explains why it shows up until we leave the target area - its
	...
	-- TODO: This always stops right away? OHHH probably because The
	-- second target area doesn't have p2... Need to account for that
	-- Maybe remove/replace second target area?
...
if isNewestVersion then
	...
	function WeaponPreview:finalizeInit()
		...
		WeaponPreview.IsSecondTargetAreaMarker = isFinalTargetMarker
